locals {
  index_content = templatefile("${path.module}/resources/redoc.html", {
    title = var.title
...
resource "aws_s3_bucket" "this" {
  bucket = "${var.name_prefix}-documentation-${data.aws_caller_identity.current.account_id}-${data.aws_region.current.region}"
